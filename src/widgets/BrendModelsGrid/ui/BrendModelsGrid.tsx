...
interface IProps {
    data: undefined | NS.IServerBrend['models']
    loading: boolean
}
export const BrendModelsGrid: FC<IProps> = memo(({ data, loading  }) => {
    ...
    type Pattern = 'small_medium' | 'medium_small' | 'small_medium_small' | 'medium' | 'large' | 'small'
    ...
    while (!(SmallCards.isFinished() && LargeCards.isFinished() && MediumCards.isFinished())) {
        ...
        const currentPattern: Pattern = availablePatterns.length > 0
            ...
            : countOfMediumCards > 0 ? 'medium_small' : 'small'
        
        carsWithProperOrder[rowIndex] = { pattern: currentPattern, data: [] }

        switch (currentPattern) {
            case 'medium':
                carsWithProperOrder[rowIndex].data = [...MediumCards.getElements(2)]
                break
            case 'small':
    ...
    return <div className={s.w}>
        { carsWithProperOrder.map((row, index) => <div key={index} className={cn(s.row, addPrefix('pattern', row.pattern, s))}>
            { row.data.map(model => <CarModelMiniCard onClick={goToModelPage(model.id)} {...model} key={model.id} />) }
        </div>) }
    </div>
})
